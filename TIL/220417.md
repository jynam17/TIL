어제 쓰던 글을 마저 쓰겠다!
# [React스럽게 생각하기](https://reactjs-kr.firebaseapp.com/docs/thinking-in-react.html) 읽으면서 [TeamMembers 컴포넌트](https://github.com/Neogasogaeseo/Naega-Web/blob/dev/src/presentation/components/TeamMembers/index.tsx) 리팩토링 구상하기!
...
<br /> 
그리고 네이밍도 여러 개 바꿀 것 같다.

시간 상의 문제로 생략한 나머지 단계들도 다음에 찬찬히 잘 읽어봐야겠다!

++)
## [이게 전부입니다.](https://reactjs-kr.firebaseapp.com/docs/thinking-in-react.html#%EC%9D%B4%EA%B2%8C-%EC%A0%84%EB%B6%80%EC%9E%85%EB%8B%88%EB%8B%A4)에서
...
코드를 쓸 일보다 읽을 일이 더 많다는 사실을 기억하라! 라니 ..
<br /> 가독성이 왜 중요한지 더 구체적으로 와닿게 됐다.
<br /> 가독성에 대한 회의는 나보다 훨씬 고차원적인 경험을 한 사람이 가지는게 맞는 것 같다.
<br /> 일단은 항상 가독성을 최대한 열심히 고려해서 코드를 짜보자!!


# TeamMembers 컴포넌트 리팩토링하기 (의식의 흐름!)
## 1. 컴포넌트 파일 새로 만들기 / 컴포넌트 네이밍 다시 하기
아예 컴포넌트 파일을 새로 만들어서 써내려가려고 한다.

그러면서 네이밍을 새로 하려는데 TeamMembers 컴포넌트의 이름을 UserSearchForTeamRegister로 바꾸려고 한다.
<br /> 참고로 팀 수정을 위한 유저 검색 뷰는 UI도, 기능도 달라서 ~ForTeamEdit 컴포넌트도 생길 것이다.

그런데 폴더 구조를
<br /> components/UserSearchForTeamRegister, components/UserSearchForTeamEdit로 할지
components/UserSearch/ForTeamRegister, components/UserSearch/ForTeamEdit로 할지 고민된다.

프로젝트 초기에 정한 폴더 구조 컨벤션이 좀 애매하진 상태다...
<br /> 팀원들에게 의견을 물어볼까???
<br /> 일단 하고싶은대로 하고 pr에 의견을 물어봐야곘다!

## 2. state/함수/기타 hook 선언 위치 정하기
- 상위 컴포넌트인 TeamRegister에서 사용되는 선택된 유저 리스트를 제외하고는 웬만하면 다 UserSearchForTeamRegister에 선언해주자!
- 선택된 유저 리스트의 prop drilling이 꽤 많은 단계를 거쳐 일어날 것 같아서 prop drilling이 몇 단계 정도 이루어지면 전역 상태를 사용하는 것이 좋을지 고민을 해보겠다.
  - 일단 prop drilling이 우려되는 이유는 너무 많은 단계를 거쳐 추적이 어렵다는 이유이다. 그래서 누군가는 3~5단계 정도면 괜찮다고 한다.
  - 내 경우는 어떨지 스스로 판단해보자!
    - 선택된 유저 리스트는 딱 팀원 등록하기 기능 안에서 최대 3단계 정도까지만 prop drilling이 되어 사용될 것 같다. 그래서 추적에 그다지 어려움이 없을 것이라 생각한다.
    - 그리고 이제 팀원 수정 기능이 새로 생길 예정이고 거기에서도 선택된 유저 리스트가 필요할텐데 전역보다는 각각 지역적으로 선언되는 것이 더 좋을 것 같다. 기능 차이 때문에 둘이 같은 타입일 것도 아니기 때문이다.
    - 전역 상태 말고 prop drilling을 하자!
